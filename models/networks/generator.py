...
from util.util import randcut,edge_conv2d,canny
...
class SPADEGenerator(BaseNetwork):
    ...
    def forward(self, input, img, z=None):
        seg0= input #label

        seg_cut=input  #将label其他两维赋值第0维的值
        seg_cut[:, 1, :, :] = seg_cut[:, 0, :, :]
        seg_cut[:, 2, :, :] = seg_cut[:, 0, :, :]
        ...
        # sobel2 = (sobel > 0.7).float()*sobel
        # sobel3 = (sobel > 0.5).float()*sobel
