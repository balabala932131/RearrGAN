...
########################
def randcut(img): #[B,C,H,W]
    b,c,h,w=img.shape
    imgnew=torch.zeros_like(img)
    size=int(h/8)
    blocks=[]
    for i in range(8):
        for j in range(8):
...
def edge_conv2d(im): #输入为[B,C,H,W]
    # 用nn.Conv2d定义卷积操作
    ...
    # print(conv_op, '\n')
    # im = torch.Tensor(im)
    edge_detect = conv_op(im.cpu()).cuda()
...
def canny(img):
    tensor_all=torch.zeros_like(img)
    B,C,H,W=img.shape
    for b in range(B):
        img_tensor=img[b,:,:,:].unsqueeze(0)
        img_BGR=image_tensor2cv2(img_tensor)
        # img_gray=cv2.cvtColor(img_BGR, cv2.COLOR_BGR2GRAY)
        img_new=np.zeros_like(img_BGR)
        img_canny=cv2.Canny(img_BGR,100,255)
        # retval, labels, stats, centroids = cv2.connectedComponentsWithStats(img_canny)
        # i = 0
        # for istat in stats:
        #     if istat[4] < 200:
        #         cv2.rectangle(img_canny, tuple(istat[0:2]), tuple(istat[0:2] + istat[2:4]), 0, thickness=-1)  # 26
        #     i = i + 1
        img_new[:, :, 0] = img_canny
        img_new[:, :, 1] = img_canny
        img_new[:, :, 2] = img_canny
        # cv2.imwrite('./data/canny1.jpg',img_new)
        Img_new=Image.fromarray(cv2.cvtColor(img_new, cv2.COLOR_BGR2RGB))
        tensor_new=transform(Img_new).unsqueeze(0).cuda()
        if b==0:
            tensor_all=tensor_new
        elif b>0:
            tensor_all=torch.cat((tensor_all,tensor_new),0)
    return tensor_all
